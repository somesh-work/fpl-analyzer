...
def load_cache():
    """Loads the prediction cache from a file on startup."""
    ...
    if os.path.exists(PREDICTION_CACHE_FILE):
        with open(PREDICTION_CACHE_FILE, 'r') as f:
            try:
                ...
                PREDICTION_CACHE = {int(k): v for k, v in PREDICTION_CACHE.items()}
            except json.JSONDecodeError:
                PREDICTION_CACHE = {}

def save_cache():
    """Saves the prediction cache to a file."""
    with open(PREDICTION_CACHE_FILE, 'w') as f:
        json.dump(PREDICTION_CACHE, f, indent=4)
...
def load_all_data(force_refresh=False):
    """
    Fetches all bootstrap and player data for the current season from the live API.
    Uses a Parquet cache for player history to dramatically speed up startup.
    """
    ...
    try:
        ...
        positions_df = pd.json_normalize(bootstrap_data['element_types'])
        ...
        players = PLAYERS_DF_FULL[['id', 'web_name', 'team', 'element_type']]
        players = players.merge(
            TEAMS_DF[['id', 'name']].rename(columns={'id': 'team_id'}),
            left_on='team',
            right_on='team_id'
        ).drop(['team', 'team_id'], axis=1)
        players = players.merge(
            positions_df[['id', 'singular_name_short']].rename(columns={'id': 'position_id'}),
        ...
        # --- PERFORMANCE ENHANCEMENT: CACHING PLAYER HISTORY ---
        if os.path.exists(ALL_DATA_CACHE_FILE) and not force_refresh:
            print(f"Loading player history from cache: {ALL_DATA_CACHE_FILE}")
            points = pd.read_parquet(ALL_DATA_CACHE_FILE)
        else:
            if force_refresh:
                print("Forcing data refresh. Fetching all player histories...")
            else:
                print("No cache found. Performing full fetch of all player histories (this will take a few minutes)...")
            
            all_histories = [get_gameweek_history(player_id) for player_id in tqdm(players['id'], desc="Fetching Player Histories")]
            points = pd.concat(all_histories, ignore_index=True)
            if not points.empty:
                points = players.merge(points, left_on='id', right_on='element')
                if 'goals_scored' in points.columns and 'assists' in points.columns:
                    points['goals_n_assists'] = points['goals_scored'] + points['assists']
                
                print(f"Saving player history to {ALL_DATA_CACHE_FILE} for faster future startups.")
...
def _get_player_predictions(gw_to_predict, custom_weights=None):
    ...
    # Use custom weights if provided, otherwise use default hardcoded weights
    if custom_weights:
        ...
        weights = {
            ...
            'gkp_xGC': 0.50, 'gkp_ppg': 0.50, 'fdr': 1.0 # Default FDR weight
        }
    ...
    def calculate_predicted_points(player):
        ...
        # Apply FDR weight to the multiplier
...
def _get_optimal_team_for_gw(gw_to_predict, is_review=False, custom_weights=None):
    ...
    if pulp.LpStatus[squad_prob.status] == 'Optimal':
        ...
        # --- LINEUP OPTIMIZATION (11 players from the 15) ---
...
def calculate_all_review_teams():
    ...
    global PREDICTED_TEAM_HISTORY, PREDICTION_CACHE
    PREDICTED_TEAM_HISTORY = []
    ...
    for gw in tqdm(range(1, latest_gw + 1), desc="Building/Reviewing History"):
        squad_df = None
        squad_for_gw_list = PREDICTION_CACHE.get(gw)
        
        if squad_for_gw_list:
            squad_df = pd.DataFrame(squad_for_gw_list)
        else:
            # If not in cache, generate it retrospectively
            print(f"No cache for GW{gw}. Generating retrospective team for history.")
            squad_df = _get_optimal_team_for_gw(gw, is_review=True)
            if squad_df is not None:
                # IMPORTANT: Save the newly generated team to the cache
                ...
                PREDICTION_CACHE[gw] = squad_df[cols_to_cache].to_dict(orient='records')
                save_cache()

        if squad_df is not None:
            # The generated df has 'now_cost', the cached one might too. Let's be consistent.
...
def get_predicted_team():
    ...
    cached_squad_list = PREDICTION_CACHE.get(next_gw)
    if cached_squad_list:
        print(f"Loading predicted team for GW{next_gw} from cache.")
        squad_df = pd.DataFrame(cached_squad_list)
        if 'now_cost' in squad_df.columns:
            squad_df.rename(columns={'now_cost': 'cost'}, inplace=True)

    # 2. IF NOT IN CACHE, CALCULATE AND SAVE
    else:
        print(f"No cache for GW{next_gw}. Generating a new prediction.")
        squad_df = _get_optimal_team_for_gw(next_gw, is_review=True)
        if squad_df is not None:
            # Save the new prediction to cache
            cols_to_cache = ['id','status', 'web_name', 'team', 'element_type', 'now_cost', 'raw_xGI', 'raw_ict_index', 'raw_ppg_last_4', 'avg_fdr', 'predicted_points']
            PREDICTION_CACHE[next_gw] = squad_df[cols_to_cache].to_dict(orient='records')
            save_cache()
            print(f"Saved prediction for GW{next_gw} to cache.")
            # Rename for processing below
            squad_df.rename(columns={'now_cost': 'cost'}, inplace=True)
...
def get_review_team():
    ...
    squad_df = None
    cached_squad_list = PREDICTION_CACHE.get(gw_to_review)
    
    if cached_squad_list:
        print(f"Loading GW{gw_to_review} prediction from cache.")
        squad_df = pd.DataFrame(cached_squad_list)
        squad_df.rename(columns={'now_cost': 'cost'}, inplace=True)
    else:
    ...
    if captain is not None:
        total_actual_points += captain['actual_points'] # Add captain's points again
...
# --- NEW: TRANSFER SUGGESTER ENDPOINT ---
...
def get_transfer_suggestions():
    ...
    try:
        ...
        # 2. Get predictions for all players for the upcoming gameweek
        all_predictions = _get_player_predictions(gw + 1)
        
        # Merge team names and positions for rich output
        ...
        # 3. Identify players to keep, sell, and buy
        ...
        # Differential suggestions (low ownership, high predicted points)
...
def force_refresh():
    try:
        ...
        if os.path.exists(ALL_DATA_CACHE_FILE):
            ...
            print(f"Deleted cache file: {ALL_DATA_CACHE_FILE}")
        
        # Reload all data, forcing a fresh download
...
def get_top_performers(df_slice, group_by_cols, agg_dict, sort_by_cols, head_n=10, ascending=False):
    ...
    # Check if all columns for aggregation exist
    for col in agg_dict.values():
        if col[0] not in df_slice.columns:
            # Return an empty DataFrame with expected columns if a required column is missing
